...
def _bars_to_seconds(bars: float, raw: RawFeatures) -> float:
...
def _time_boxed_cue(
    ...
    target = reference_position + _bars_to_seconds(config.time_box_bars, raw)
...
def emit_cues(
    ...
    if structure_template == "edm":
        if raw.riser_candidates and raw.riser_candidates[0].start_time < grid_start:
            ...
            cue_ins.append(
                Cue(position=riser.start_time, kind="riser_start", confidence=riser.confidence)
            )
        else:
            cue_ins.append(
    ...
    # time_boxed is anchored to this module's own top-preference cue-in
    # candidate (cue_ins[0], always present) — spec's formula assumes a
    # singular "cue-in" reference point, which predates this module's
    # multi-candidate emission design.
    time_boxed = _time_boxed_cue(raw, cue_ins[0].position, phrase_grid, config)
    if time_boxed is not None:
        cue_outs.append(time_boxed)

    # Hard constraint, unconditional (§1, §7, D9/D10): no cue before
